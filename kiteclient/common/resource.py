...
class Resource(collections.MutableMapping):
    ...
    # the attribute that is used for unique identification
    id_key = 'id'
    ...
    def __init__(self, attrs=None, loaded=False):
        ...
        self._id = attrs.pop(self.id_key, None)
        self._attrs = attrs
    ...
    def id(self, value):
        if self._id and self._id != value:
            raise AttributeError('You cannot change an existing ID')
    ...
    def create(self, session):
        ...
        self._id = resp_body.pop(self.id_key, None)
        self._attrs.update(resp_body)
        self._reset_dirty()
    ...
    def update(self, session):
        ...
        try:
            resp_id = resp_json.pop(self.id_key)
        except KeyError:
            pass
        else:
    ...
    def _http_request(cls, session, method, path, **kwargs):
        ...
        if cls.service_type:
            endpoint_filter.setdefault('service_type', cls.service_type)
